 ...
 ******************************************************************************/
package pl.luckboy.logicalterm.range3
...
class MatchingTermMatcher extends Matcher[MatchingTerm]
{
  ...
  private def distributeSuperconjunctionNode(node: TermNode, rangeSets: Map[String, TermNodeRangeSet], depthRangeSets: List[TermNodeRangeSet], isRoot: Boolean)(varIdx: Int): List[(Option[TermNodeRangeSet], TermNode)] = {
    val depthRangeSet = depthRangeSets.headOption.getOrElse(TermNodeRangeSet.empty)
    (node match {
      case TermBranch(childs, _) =>
        val (_, pairs6) = childs.foldLeft((varIdx, List[(Option[TermNodeRangeSet], TermNode)]())) {
          case ((newVarIdx, pairs), child) =>
            val pairs2 = distributeSuperdisjunctionNode(child, rangeSets, depthRangeSets, false)(newVarIdx)
            if(!pairs.isEmpty) {
              ...
              (newVarIdx + child.varCount, pairs2.zipWithIndex.flatMap { case ((ors, n), pi) => if(!pairIdxs3.contains(pi)) List((ors, TermBranch(Vector(n), n.varCount))) else Nil } ++ pairs5)
            } else
              (newVarIdx + child.varCount, pairs2.map { case (ors, n) => (ors, TermBranch(Vector(n), n.varCount)) })
        }
        if(isRoot)
          List(pairs6.foldLeft((some(TermNodeRangeSet.empty), TermBranch(Vector(), 0))) {
            case ((ors, n), (ors2, n2)) => ((ors |@| ors2) { _ & _ }, n &| n2)
          })
        else
          pairs6
      case TermLeaf(_) =>
        distributeSuperdisjunctionNode(node, rangeSets, depthRangeSets, false)(varIdx)
    }).map { case (ors, n) => (ors.map { _.superset(depthRangeSet) }, n.normalizedTermNode) }
  }

  private def distributeSuperdisjunctionNode(node: TermNode, rangeSets: Map[String, TermNodeRangeSet], depthRangeSets: List[TermNodeRangeSet], isRoot: Boolean)(varIdx: Int): List[(Option[TermNodeRangeSet], TermNode)] = {
    ...
    (node match {
      case TermBranch(childs, _) =>
        childs.foldLeft((varIdx, List[(Option[TermNodeRangeSet], TermNode)]())) {
          case ((newVarIdx, pairs), child) =>
            val pairs2 = distributeSuperconjunctionNode(child, rangeSets, depthRangeSets2, isRoot)(newVarIdx)
            val pairs3 = if(!pairs.isEmpty)
            ...
            (newVarIdx + child.varCount, pairs3)
        }._2
  ...
  private def distributedTermFromSuperconjunctionNode(node: TermNode, rangeSets: Map[String, TermNodeRangeSet], depthRangeSets: List[TermNodeRangeSet], varArgs: Map[String, Vector[MatchingTerm]]) =
    distributeSuperconjunctionNode(node, rangeSets, depthRangeSets, true)(0) match {
      case List((_, newNode)) => some(MatchingTerm(newNode.withIndexes, varArgs))
      case pairs              => none
    }
  ...
  private def distributedTermFromSuperdisjunctionNode(node: TermNode, rangeSets: Map[String, TermNodeRangeSet], depthRangeSets: List[TermNodeRangeSet], varArgs: Map[String, Vector[MatchingTerm]]) =
    distributeSuperdisjunctionNode(node, rangeSets, depthRangeSets, true)(0) match {
      case List((_, newNode)) => some(MatchingTerm(newNode.withIndexes, varArgs))
      case pairs              => none
    }
  ...
  private def distributeSuperconjunctionNodeAndDisjunctionNodeFromTerms(term1: MatchingTerm, term2: MatchingTerm, isFirstTry: Boolean) =
    (term1.conjNode, term2.conjNode) match {
      case (TermBranch(childs1, _), TermBranch(childs2, _)) if childs1.size > 1 && childs2.size === 1 && !isFirstTry =>
        ...
        for {
          distributedTerm1 <- distributedTermFromSuperconjunctionNode(TermBranch(Vector(TermBranch(Vector(term1.conjNode), term1.conjNode.varCount)), term1.conjNode.varCount), term2.info.conjRangeSets, conjDepthRangeSets, term1.varArgs)
          distributedTerm2 <- distributedTermFromSuperdisjunctionNode(term2.conjNode, term1.info.disjRangeSets, disjDepthRangeSets, term2.varArgs)
      ...
      case (TermBranch(childs1, _), TermBranch(childs2, _)) if childs1.size === 1 && childs2.size > 1 && isFirstTry =>
        ...
        for {
          distributedTerm1 <- distributedTermFromSuperconjunctionNode(term1.conjNode, term2.info.conjRangeSets, conjDepthRangeSets, term1.varArgs)
          distributedTerm2 <- distributedTermFromSuperdisjunctionNode(TermBranch(Vector(TermBranch(Vector(term2.conjNode), term2.conjNode.varCount)), term2.conjNode.varCount), term1.info.disjRangeSets, disjDepthRangeSets, term2.varArgs)
        } yield (distributedTerm1, distributedTerm2)
      case _ =>
        val conjDepthRangeSets = TermNodeRangeSet.full :: term2.info.conjDepthRangeSets
        val disjDepthRangeSets = TermNodeRangeSet.full :: TermNodeRangeSet.full :: term1.info.disjDepthRangeSets
  ...
  private def checkSuperconjunctionNodeAndDisjunctionNodeFromTerms(term1: MatchingTerm, term2: MatchingTerm, isFirstTry: Boolean) =
    distributeSuperconjunctionNodeAndDisjunctionNodeFromTerms(term1, term2, isFirstTry).map {
      case (distributedTerm1, distributedTerm2) =>
        ...
        val conjRangeSet = checkSuperconjunctionNode(distributedTerm1.conjNode, distributedTerm2.info.conjRangeSets, conjDepthRangeSets)(0)
        val disjRangeSet = checkSuperdisjunctionNode(distributedTerm2.conjNode, distributedTerm1.info.disjRangeSets, disjDepthRangeSets)(0)
